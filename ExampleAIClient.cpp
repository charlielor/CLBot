...
int main(int argc, const char* argv[]) {
	...
	while (true) {
		...
		if (Broodwar->isReplay()) {
			...
			static int numberOfStartingMineralPatches = Broodwar->getStaticMinerals().size();
			static int numberOfStartingGasGeysers = Broodwar->getStaticGeysers().size();
			...
			for (auto& u : units) {
				if (u->getType().isWorker()) {
					...
					if (numberOfWorkers <= numberOfStartingMineralPatches) {
						Broodwar << "Number of Workers: " << numberOfWorkers << std::endl;

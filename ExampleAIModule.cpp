...
void ExampleAIModule::onFrame()
{
	...
	for (auto& u : Broodwar->self()->getUnits())
	{
		...
		static bool barracks = false;

		// If the unit is a worker unit
		if (u->getType().isWorker())
		{
			// If supply count is 10, determine offensive unit building and build it
			static int lastOffensiveBuildingCheck = 0;

			if (Broodwar->self()->supplyUsed() % 10 == 0 && Broodwar->self()->minerals() >= UnitTypes::Terran_Barracks.mineralPrice() && lastOffensiveBuildingCheck + 500 < Broodwar->getFrameCount()) {
